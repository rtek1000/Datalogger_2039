...
#include "cmsis_os.h"
#else
#define	W25qxx_Delay(delay)		IWDG_delay_ms(delay)
#endif
//###################################################################################################################
...
//		W25qxx_Delay(1);
		...
		W25qxx_Delay(0); // MX_USB_HOST_Process_FS();
//		UsrLog("W25qxx_WaitForWriteEnd");
...
#if (_W25QXX_DEBUG==1)
	printf("w25qxx Page Size: %lu Bytes\r\n", w25qxx.PageSize);
...
#if (_W25QXX_DEBUG==1)
	uint32_t StartTime = HAL_GetTick();
	printf("w25qxx WriteByte 0x%02X at address %lu begin...", pBuffer,
			WriteAddr_inBytes);
#endif
	W25qxx_WaitForWriteEnd();
	W25qxx_WriteEnable();
...
void W25qxx_WriteSectorZeroFill(uint32_t Sector_Address) {
	uint8_t pBuffer[256];
	uint16_t NumByteToWrite_up_to_SectorSize = 4096;
	uint8_t OffsetInByte = 0;
	for (int i = 0; i < 256; i++) {
		pBuffer[i] = 0;
	}
	if ((NumByteToWrite_up_to_SectorSize > w25qxx.SectorSize)
...
void W25qxx_WriteBlockZeroFill(uint32_t Block_Address) {
	uint8_t pBuffer[256];
	uint32_t NumByteToWrite_up_to_BlockSize = 65536;
	uint8_t OffsetInByte = 0;
